...
use crate::atividades_extensao_model::{AtividadesExtensao, InsertableAtividadeExtensao};
use crate::schema;
use crate::DbConn;
...
pub fn read_atividades_extensao(
    id_professor: i32,
    conn: DbConn,
) -> Result<Json<Vec<AtividadesExtensao>>, String> {
    schema::atividades_extensao::table
        .filter(schema::atividades_extensao::id_professor.eq(id_professor))
        .load(&conn.0)
        .map_err(|err| -> String {
